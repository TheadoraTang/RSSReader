...
- 手动添加订阅和 OPML 导入都会尝试首次同步；首次同步失败时保留订阅源，并返回具体失败原因供用户重试。
...
- 导入后立即解析 Feed 并保存文章。
- 如果订阅源已添加但首次同步失败，结果状态为 `partial`，订阅源会保留在列表中，用户可稍后再次同步。
- 已存在的订阅会跳过。
- 无效 URL 不会创建订阅源，会记录失败结果和原因。

响应字段：

- `total`: 本次识别出的订阅总数
- `imported`: 新增并同步成功数量
- `partial`: 已新增但首次同步失败数量
- `skipped`: 已存在或跳过数量
- `failed`: 失败数量
- `results`: 每条订阅的处理结果
...
- `status`: `success` 表示已添加并同步成功，`partial` 表示订阅源已添加但首次同步失败
- `message`: 同步成功提示或失败原因
- `feed`: 已创建的订阅源

当返回 `partial` 时，订阅源仍保留在数据库中，后续可以通过 `POST /api/feeds/{feed_id}/sync` 或 `POST /api/feeds/sync-all` 重试。
...
- 导入完成后刷新订阅列表。
- 根据导入结果显示成功、已添加但同步失败、跳过或失败提示，并展示每条失败原因与建议。
...
- 重复 `xmlUrl` 去重。
- OPML 导入中的新增、跳过和失败结果。
- 手动添加订阅的首次同步成功和首次同步失败保留订阅源。
- OPML 导入中已新增但首次同步失败的 `partial` 结果。
- OPML 导出读取当前订阅源。
...
- OPML 导入会立即同步，遇到慢速或不可访问 Feed 时导入完成时间会受影响；不可访问的 Feed 会以 `partial` 状态保留，方便后续重试。
